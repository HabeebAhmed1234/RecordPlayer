...
public class PlayPauseDrawable extends Drawable {
    public enum STATE {STATE_PAUSED, STATE_PLAYING}
    ...
    private STATE mCurrentState = STATE.STATE_PLAYING;

    public PlayPauseDrawable(Context context) {
        this(60, context.getResources().getColor(R.color.play_colour), context.getResources().getColor(R.color.pause_colour), 300);
    }
    ...
    public void toggle() {
        if(mCurrentState == STATE.STATE_PAUSED) {
            animatePause();
            mCurrentState = STATE.STATE_PLAYING;
        } else {
            animatePlay();
    ...
    public void setCurrentState(STATE state) {
        if (state != mCurrentState) {
            toggle();
        } else if(mCurrentState == STATE.STATE_PAUSED) {
            animatePlay();
        } else if(mCurrentState == STATE.STATE_PLAYING) {
            animatePause();
        }
    }
